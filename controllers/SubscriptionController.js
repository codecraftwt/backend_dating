...
const getSubscriptionStatus = async (req, res) => {
    try{
        ...
        res.status(StatusCodes.OK).json({ subscription: user.subscription, message: 'Subscription status fetched successfully', status: StatusCodes.OK, success: true });
    } catch(error) {
        console.error("Error fetching subscription status:", error);
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).send({ error: { message: "Failed to fetch subscription status" } });
    }
}

module.exports = { createCustomer, createSubscription, createSubscriptionPlan, getAllSubscriptionPlans, getSubscriptionStatus };